...
function checkAuthUI() {
  ...
  if (token && userStr) {
    ...
    if ($('nav-register-btn')) $('nav-register-btn').style.display = 'none';
    
    // Show the user menu (My Courses + Profile Pill)
    if ($('user-menu')) $('user-menu').style.display = 'flex';
    
    // Set Profile Details in the Pill
    ...
    // Set Profile Details in the Popup
    if ($('popup-header-name')) $('popup-header-name').textContent = user.full_name;
    if ($('popup-header-email')) $('popup-header-email').textContent = user.email || "No email provided";
    ...
    // Hide the user menu and popup
    if ($('user-menu')) $('user-menu').style.display = 'none';
    if ($('header-user-popup')) $('header-user-popup').style.display = 'none';
  }
}

// Toggle the header user popup
function toggleHeaderUserPopup() {
    const popup = $('header-user-popup');
...
document.addEventListener('click', function(event) {
    ...
    const popup = $('header-user-popup');
    if (userMenu && !userMenu.contains(event.target) && popup) {
        popup.style.display = 'none';
    }
});
// ── LOGIN MODAL LOGIC ──────────────────────────────────────────────────────
...
function confirmLogout() {
  // Hide the profile popup menu if it's open
  const popup = $('header-user-popup');
  if (popup) popup.style.display = 'none';

  // Show the "Are you sure?" warning modal
